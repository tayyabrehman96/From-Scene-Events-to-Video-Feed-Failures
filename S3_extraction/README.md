...
`core_primary_evidence.csv` is a broader 52-study extraction of all manuscript comparison / SOTA tables. `appraisal_aggregate.csv` records the article's proposed appraisal numbers for 132 core studies (660 D1–D5 ratings, 91.2% agreement, κ = 0.867, 95% CI [0.832, 0.898]). These originated from a provisional scenario, and the row-level rating matrix is absent; therefore they are not source-verified review results.
